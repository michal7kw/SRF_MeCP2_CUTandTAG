...
def init_worker(bigwig_path):
    ...
    try:
        ...
        # Log error from worker process initialization if needed
        # logger.error(f"Worker failed to open BigWig {bigwig_path}: {e}")
        bw_reader = None # Ensure it's None if opening fails
...
def extract_signal_for_region(args):
    ...
    # Check if the bigwig reader was initialized successfully in this worker
    if bw_reader is None:
        # logger.error("BigWig reader not initialized in worker.")
    ...
    try:
        ...
        if chrom not in bw_reader.chroms():
            # logger.warning(f"Chromosome {chrom} not found in {os.path.basename(bigwig_path)}")
            return np.full(n_bins, np.nan) # Return NaNs if chromosome not found

        # Check if the window is valid within the chromosome limits
        chrom_len = bw_reader.chroms(chrom)
        if start >= chrom_len:
            # logger.warning(f"Start coordinate {start} beyond chromosome {chrom} length {chrom_len}")
             return np.full(n_bins, np.nan) # Return NaNs if start is beyond chromosome length

        # Adjust end coordinate if it exceeds chromosome length
        end = min(end, chrom_len)

        if start >= end:
            # logger.warning(f"Invalid window coordinates after adjustment: {chrom}:{start}-{end}")
            return np.full(n_bins, np.nan) # Return NaNs for invalid coordinates

        values = bw_reader.stats(chrom, start, end, type="mean", nBins=n_bins)
        # Replace None values (missing data) with np.nan
        signal = [v if v is not None else np.nan for v in values]
        return np.array(signal)

    except Exception as e:
        # logger.error(f"Error processing region {chrom}:{start}-{end} in {os.path.basename(bigwig_path)}: {str(e)}")
