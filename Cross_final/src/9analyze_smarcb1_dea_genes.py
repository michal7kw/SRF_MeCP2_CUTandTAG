...
def main():
    # Input files
    dea_file = os.path.join(config.DATA_DIR, "DEA_NSC.csv")
    gtf_file = config.GTF_FILE
    smarcb1_dir = config.SMARCB1_DIR
    output_dir = os.path.join(config.RESULTS_DIR, "9analyze_smarcb1_dea_genes")
    
    # Load data
    dea_genes = load_dea_genes(dea_file)
    gene_annotations = load_gene_annotations(gtf_file)
    
    # Analyze SMARCB1 levels
    results = analyze_smarcb1_levels(dea_genes, gene_annotations, smarcb1_dir)
