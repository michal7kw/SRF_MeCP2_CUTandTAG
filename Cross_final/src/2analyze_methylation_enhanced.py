...
class EnhancedProfileAnalyzer:
        ...
        with pyBigWig.open(ip_file) as ip_bw, pyBigWig.open(input_file) as input_bw:
            for i, (_, region) in enumerate(regions.iterrows()):
                try:
                    ...
                    # GC content: percentage of bases in a DNA sequence that are either guanine or cytosine --> Number of G or C bases / Total number of bases
                    # CpG density: frequency of CpG dinucleotides --> Number of CpG dinucleotides / Total number of C or G bases
    ...
    def plot_comparison_across_categories(self, profiles: Dict[str, np.ndarray], 
                                        title: str, ylabel: str, output_path: str):
        """Plot profiles from different categories on the same graph."""
        x = np.linspace(-self.window_size, self.window_size, self.n_bins)
        
        plt.figure(figsize=(12, 6))
        colors = {'up': 'red', 'down': 'blue', 'no_deg': 'gray'}
        
        for category, profile in profiles.items():
            plt.plot(x, profile, label=category.capitalize(), color=colors[category])
...
def main():
    ...
    for category in ['up', 'down', 'no_deg']:
        ...
        # Create histogram of region lengths
        print("Creating histogram of region lengths...")
        region_lengths = regions['end'] - regions['start']
        plt.figure(figsize=(10, 6))
        plt.hist(region_lengths, bins=50, edgecolor='black')
        plt.title(f'Distribution of {category.capitalize()} Region Lengths')
        plt.xlabel('Region Length (bp)')
