...
def calculate_methylation_levels_with_replicates(region_df: pd.DataFrame,
    ...
    """Calculate methylation levels using improved window-based analysis"""
    cell_type_map = {
        'NEU': 'N',
        'NSC': 'PP'
    }
    ...
    for rep_file, input_file in zip(replicate_files, input_files):
        ...
        try:
            ...
            batches = range(0, total_regions, window_size)
            ...
            with tqdm(total=total_regions, desc=f"Calculating methylation for {os.path.basename(rep_file)}") as pbar:
                for start_idx in batches:
                    end_idx = min(start_idx + window_size, total_regions)
                    ...
                    for _, row in batch.iterrows():
                        try:
                            ...
                            # Process region in windows
                            for start in range(int(row['start']), int(row['end']), window_size):
                                end = min(start + window_size, int(row['end']))
                                
                                # Get values for this window
                                ip_window = bw_ip.values(row['chr'], start, end)
                                input_window = bw_input.values(row['chr'], start, end)
                                sequence = fasta.fetch(row['chr'], start, end)
                                
                                if ip_window and input_window:
                                    # Calculate methylation for this window
                                    window_methylation = calculate_normalized_methylation(
                                        ip_window, input_window, sequence
                                    )
                                    if window_methylation > 0:  # Only include windows with signal
                                        methylation_windows.append(window_methylation)
                            
                            # Calculate final methylation level
                            if methylation_windows:
                                # Use median to be more robust to outliers
                                methylation = np.median(methylation_windows)
                                
                                # Add quality metrics
                                coverage = len(methylation_windows) / ((row['end'] - row['start']) / window_size)
                                variation = np.std(methylation_windows) / np.mean(methylation_windows) if len(methylation_windows) > 1 else 1
                                
                                # Adjust confidence based on coverage and variation
                                confidence = min(coverage, 1.0) * (1 - min(variation, 1.0))
                            else:
                                methylation = 0
                                confidence = 0
                            ...
                            batch_confidence.append(0)
                    
                    methylation_values.extend(batch_methylation)
                    confidence_values.extend(batch_confidence)
    ...
    return pd.DataFrame({
        ...
        'confidence': confidence_values if confidence_values else [0] * len(avg_methylation)
    })
...
def calculate_normalized_methylation(ip_values, input_values, sequence):
    ...
    # Calculate enrichment over input
    ip_mean = np.mean(ip_values)
    input_mean = np.mean(input_values) if np.mean(input_values) > 0 else 1.0
    
    # Calculate normalized enrichment score
    enrichment = ip_mean / input_mean
    
    # Convert to methylation percentage with proper scaling
    # Typically MeDIP signal follows a non-linear relationship with methylation levels
    methylation = 100 * (1 - 1/(1 + enrichment))
    
    # Ensure value is between 0-100
    return min(100, max(0, methylation))
...
def calculate_enhanced_confidence_score(methylation: float, 
    ...
    """Calculate confidence score with very lenient thresholds for testing"""
    # Extremely lenient CpG threshold
    min_cpgs = 1  # Accept even single CpG regions for testing
    cpg_score = 1.0 if cpg_count >= min_cpgs else cpg_count
    
    # Very lenient density threshold
    density_score = min(1.0, cpg_density / 0.0005)  # Even lower density requirement
    
    # Simple confidence score focusing on presence of CpGs
    confidence = (
        0.7 * cpg_score +          # Mainly care about CpG presence
        0.3 * density_score        # Minor weight on density
    )
    
    return max(0.1, min(1.0, confidence))  # Minimum confidence of 0.1
...
def map_binding_to_genes(mecp2_binding: pd.DataFrame, genes_df: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        ...
        if not genes_df['chr'].str.startswith('chr').all():
            genes_df['chr'] = 'chr' + genes_df['chr']
        ...
        logger.info("Coordinate ranges after standardization:")
        logger.info(f"Binding regions: {mecp2_binding['chr'].nunique()} chromosomes")
        logger.info(f"Genes: {genes_df['chr'].nunique()} chromosomes")
...
def calculate_contextual_methylation(merged_df: pd.DataFrame,
                                   ...
                                   genome_fasta: str,
                                   window_size: int = 50) -> pd.DataFrame:
    """Calculate methylation with improved biological context and smoothing"""
    try:
        # Load MeDIP signal data
        bigwig_file = os.path.join(medip_dir, f"{cell_type}.bw")
        bw = pyBigWig.open(bigwig_file)
        
        # Initialize columns for methylation values
        merged_df['promoter_methylation'] = np.nan
        merged_df['gene_body_methylation'] = np.nan
        
        for idx, row in merged_df.iterrows():
            try:
                # Get promoter region
                promoter_start = row['start'] - 2000
                promoter_end = row['start'] + 500
                
                # Get gene body region
                gene_body_start = row['start'] + 500
                gene_body_end = row['end']
                
                # Calculate methylation with sliding windows
                promoter_values = []
                gene_body_values = []
                
                # Promoter methylation
                for start in range(promoter_start, promoter_end, window_size):
                    end = min(start + window_size, promoter_end)
                    try:
                        values = bw.values(row['chr'], start, end)
                        values = [v for v in values if v is not None]
                        if values:
                            # Convert signal to methylation percentage with proper scaling
                            methylation = np.mean(values) * 100 / bw.header().get('maxVal', 1.0)
                            promoter_values.append(min(100, max(0, methylation)))
                    except:
                        continue
                
                # Gene body methylation
                for start in range(gene_body_start, gene_body_end, window_size):
                    end = min(start + window_size, gene_body_end)
                    try:
                        values = bw.values(row['chr'], start, end)
                        values = [v for v in values if v is not None]
                        if values:
                            methylation = np.mean(values) * 100 / bw.header().get('maxVal', 1.0)
                            gene_body_values.append(min(100, max(0, methylation)))
                    except:
                        continue
                
                # Apply Gaussian smoothing to reduce artificial clustering
                if promoter_values:
                    smoothed_promoter = gaussian_filter1d(promoter_values, sigma=1)
                    merged_df.at[idx, 'promoter_methylation'] = np.mean(smoothed_promoter)
                
                if gene_body_values:
                    smoothed_gene_body = gaussian_filter1d(gene_body_values, sigma=1)
                    merged_df.at[idx, 'gene_body_methylation'] = np.mean(smoothed_gene_body)
                
            except Exception as e:
                logger.warning(f"Error processing gene {row['gene_id']}: {str(e)}")
                continue
        
        bw.close()
        
        # Add quality metrics
        merged_df['methylation_confidence'] = calculate_methylation_confidence(merged_df)
...
def calculate_methylation_confidence(df: pd.DataFrame) -> pd.Series:
    """Calculate confidence scores for methylation values"""
    confidence = pd.Series(index=df.index, dtype=float)
    
    for idx, row in df.iterrows():
        # Check for extreme clustering
        promoter_val = row['promoter_methylation']
        gene_body_val = row['gene_body_methylation']
        
        # Lower confidence if values are suspiciously clustered
        if abs(promoter_val - round(promoter_val)) < 0.01 or abs(gene_body_val - round(gene_body_val)) < 0.01:
            confidence[idx] = 0.5
        else:
            confidence[idx] = 1.0
...
def plot_methylation_patterns(results: Dict[str, pd.DataFrame], output_dir: str):
    """Create methylation pattern plots with improved filtering"""
    for cell_type, df in results.items():
        # Apply quality filtering
        filtered_df = filter_methylation_data(df)
        
        # Create plots
        fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(15, 6))
        
        # Plot methylation distributions with kernel density estimation
        sns.histplot(data=filtered_df, x='promoter_methylation',
                    bins=50, stat='density', kde=True, ax=ax1)
        ax1.set_title(f'{cell_type}: Promoter Methylation Distribution')
        
        sns.histplot(data=filtered_df, x='gene_body_methylation',
                    bins=50, stat='density', kde=True, ax=ax2)
        ax2.set_title(f'{cell_type}: Gene Body Methylation Distribution')
        
        # Add statistics to plot
        for ax, col in [(ax1, 'promoter_methylation'), (ax2, 'gene_body_methylation')]:
            stats_text = f'n = {len(filtered_df)}\n'
            stats_text += f'mean = {filtered_df[col].mean():.1f}\n'
            stats_text += f'median = {filtered_df[col].median():.1f}'
            ax.text(0.95, 0.95, stats_text, transform=ax.transAxes,
                   verticalalignment='top', horizontalalignment='right',
                   bbox=dict(facecolor='white', alpha=0.8))
        
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, f'{cell_type}_methylation_patterns.pdf'))
        plt.close()

def filter_methylation_data(df: pd.DataFrame, min_confidence: float = 0.6) -> pd.DataFrame:
    """Filter methylation data based on quality metrics"""
    # Remove low confidence measurements
    high_conf = df[df['methylation_confidence'] >= min_confidence].copy()
    
    # Remove suspicious patterns
    high_conf = high_conf[
        # Remove perfect multiples of 5 or 10 (likely artifacts)
        ~(high_conf['promoter_methylation'] % 5 == 0) &
        ~(high_conf['gene_body_methylation'] % 5 == 0) &
        
        # Remove extreme values without supporting evidence
        ((high_conf['promoter_methylation'] <= 90) | 
         (high_conf['cpg_count'] >= 5)) &
        
        # Remove regions with insufficient coverage
        (high_conf['cpg_count'] >= 3)
    ]
    
    return high_conf

def analyze_binding_enrichment_groups(df: pd.DataFrame, output_dir: str):
    """Analyze gene regulation patterns in different binding enrichment groups"""
    
    # 1. Categorize genes by binding type
    binding_groups = {
        'exo_only': df[df['binding_type'] == 'exo'],
        'exo_enriched': df[(df['binding_type'] == 'both') & 
    ...
    for group_name, group_df in binding_groups.items():
        ...
        promoter_genes = group_df[group_df['location'] == 'promoter']
        
        # Analyze regulation status
        regulation_counts = promoter_genes['expression_status'].value_counts()
        
        # Calculate methylation statistics for each regulation category
        methylation_stats = promoter_genes.groupby('expression_status').agg({
    ...
    # Save results
    output_file = os.path.join(output_dir, 'binding_enrichment_analysis.txt')
    with open(output_file, 'w') as f:
        for group_name, group_results in results.items():
            ...
            f.write("\nRegulation Counts:\n")
            f.write(str(group_results['regulation_counts']))
            
            f.write("\n\nMethylation Statistics:\n")
            f.write(str(group_results['methylation_stats']))
