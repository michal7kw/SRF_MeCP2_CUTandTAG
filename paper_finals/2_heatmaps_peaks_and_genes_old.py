...
"""
This script performs MeCP2 profiling around TSS regions, gene bodies, and peaks using bigWig files.
It calculates and visualizes the average signal intensity of MeCP2 around these genomic features for different sample groups.
...
def extract_signal_reference_point(args):
    """
    Extract signal from a bigWig file for a given region around a reference point (TSS or peak center).

    Args:
        args (tuple): A tuple containing the following arguments:
            bw_file (str): Path to the bigWig file.
            chrom (str): Chromosome name.
            center (int): Center coordinate of the region.
            upstream (int): Distance upstream from the center.
            downstream (int): Distance downstream from the center.
            bin_size (int): Size of each bin in base pairs.

    Returns:
        numpy.ndarray: A numpy array containing the average signal intensity for each bin.
                       Returns a zero-filled array if an error occurs.
    """
    ...
    try:
        ...
        # Calculate start and end positions of the region
        start = max(0, center - upstream)  # Ensure start is not negative
        end = center + downstream
        
        # Calculate bins using linspace
        ...
        for i in range(len(bins) - 1):
            ...
            try:
                # Get mean signal for the bin using pyBigWig's stats function
                signal[i] = bw.stats(chrom, bin_start, bin_end, type="mean")[0] or 0  # Returns None if no data
            except Exception as e:
                logger.error(f"Error extracting signal from bin {i} in {bw_file} at {chrom}:{bin_start}-{bin_end}: {str(e)}")
                signal[i] = 0  # Assign 0 if there's an error
...
def extract_signal_scale_regions(args):
    ...
    The gene body region is divided into a fixed number of bins, and the signal is averaged within each bin.
    Upstream and downstream regions are also extracted.
    ...
    Args:
        args (tuple): A tuple containing the following arguments:
            ...
            end (int): End coordinate of the gene (TES).
            upstream (int): Distance upstream from the TSS.
            downstream (int): Distance downstream from the TES.
            body_bins (int): Number of bins to divide the gene body into.
            bin_size (int): Size of each bin in the upstream and downstream regions.
...
def compute_matrix_reference_point(bw_files, regions, upstream=5000, downstream=5000, bin_size=50, n_processes=8):
    """
    Compute a signal matrix for multiple bigWig files and regions around a reference point (TSS or peak center).

    This function extracts signal from multiple bigWig files for a set of genomic regions centered around a reference point.
    It uses parallel processing to speed up the computation.

    Args:
        bw_files (list): A list of paths to the bigWig files.
        regions (pybedtools.BedTool): A BedTool object containing the genomic regions.
        upstream (int): Distance upstream from the reference point.
        downstream (int): Distance downstream from the reference point.
        bin_size (int): Size of each bin in base pairs.
        n_processes (int): Number of parallel processes to use.

    Returns:
        numpy.ndarray: A 3D numpy array of shape [n_files, n_regions, n_bins] containing the signal matrix.
    """
    ...
    # Process in parallel using a Pool of workers
    with Pool(processes=n_processes) as pool:
        # imap applies the function to each element of the iterable and returns an iterator
...
def compute_matrix_scale_regions(bw_files, regions, upstream=5000, downstream=5000, body_bins=100, bin_size=50, n_processes=8):
    """
    Compute a signal matrix for multiple bigWig files and gene bodies with scaled body length.

    This function extracts signal from multiple bigWig files for a set of gene regions (TSS to TES).
    The gene body region is scaled to a fixed number of bins.
    It uses parallel processing to speed up the computation.

    Args:
        bw_files (list): A list of paths to the bigWig files.
        regions (pybedtools.BedTool): A BedTool object containing the gene regions.
        upstream (int): Distance upstream from the TSS.
        downstream (int): Distance downstream from the TES.
        body_bins (int): Number of bins to divide the gene body into.
        bin_size (int): Size of each bin in the upstream and downstream regions.
...
def plot_tss_profile(matrices, labels, colors, output_file, title="MeCP2 around TSS",
    ...
    This function generates a line plot showing the average signal intensity around the TSS for different sample groups.

    Args:
        matrices (list): A list of numpy arrays, where each array represents the average signal intensity for a sample group.
        labels (list): A list of labels for each sample group.
        colors (list): A list of colors for each sample group.
...
def plot_gene_body_profile(matrices, labels, colors, output_file, title="MeCP2 gene body coverage",
    ...
    This function generates a line plot showing the average signal intensity across the gene body (TSS to TES)
    for different sample groups. The gene body region is scaled to a fixed number of bins.
...
def plot_side_by_side_tss(matrices, labels, colors, output_file, title_prefix="MeCP2"):
    ...
    This function generates a figure with multiple subplots, each showing the average signal intensity around the TSS
    for a different sample group.
...
def plot_side_by_side_gene_body(matrices, labels, colors, output_file, title_prefix="MeCP2"):
    ...
    This function generates a figure with multiple subplots, each showing the average signal intensity across the gene body
    for a different sample group.
...
def load_peaks(peaks_dir, sample_name):
    ...
    This function searches for peak files (narrowPeak or broadPeak) in the specified directory
    and loads them using BedTool.

    Args:
        peaks_dir (str): Path to the directory containing the peak files.
...
def debug_matrices(matrices, sample_names, output_dir):
    """
    Debug function to check matrix values at TSS and TES.

    This function extracts the signal intensity at the TSS and TES positions from the gene body matrices
    and saves them to a CSV file. It also generates plots of the raw signal around the TSS and TES.
    ...
    df = pd.DataFrame({
        'Sample': sample_names,
        'TSS_Signal': [matrix[tss_idx] for matrix in matrices],
        'TES_Signal': [matrix[tes_idx] for matrix in matrices],
        'TSS/TES_Ratio': [matrix[tss_idx]/matrix[tes_idx] if matrix[tes_idx] > 0 else 0 for matrix in matrices]
    })
    
    # Save to file
    df.to_csv(os.path.join(output_dir, 'debug_tss_tes_signals.csv'), index=False)
    logger.info(f"Debug information saved to {os.path.join(output_dir, 'debug_tss_tes_signals.csv')}")
    
    # Also save raw signal values around TSS and TES
    tss_window = 20  # Bins to check around TSS
    tes_window = 20  # Bins to check around TES
    
    for i, (matrix, name) in enumerate(zip(matrices, sample_names)):
        # Extract signal around TSS and TES
        tss_signal = matrix[max(0, tss_idx-tss_window):min(len(matrix), tss_idx+tss_window)]
        tes_signal = matrix[max(0, tes_idx-tes_window):min(len(matrix), tes_idx+tes_window)]
        
        # Save to file
        tss_df = pd.DataFrame({
            'Position': range(-tss_window, tss_window),
            'Signal': tss_signal
        })
        tss_df.to_csv(os.path.join(output_dir, f'{name}_tss_signal.csv'), index=False)
        
        tes_df = pd.DataFrame({
            'Position': range(-tes_window, tes_window),
            'Signal': tes_signal
        })
        tes_df.to_csv(os.path.join(output_dir, f'{name}_tes_signal.csv'), index=False)
    
    # Create a debug plot of raw signals
    plt.figure(figsize=(15, 10))
    for i, (matrix, name) in enumerate(zip(matrices, sample_names)):
        plt.subplot(2, 2, i+1)
        plt.plot(matrix)
        plt.title(f"{name} Raw Signal")
        plt.axvline(x=tss_idx, color='red', linestyle='--', label='TSS')
        plt.axvline(x=tes_idx, color='green', linestyle='--', label='TES')
        plt.legend()
    plt.tight_layout()
...
def main():
    ...
    This function parses command-line arguments, loads genomic regions and bigWig files,
    computes signal matrices, generates profile plots, and saves the results to the output directory.
    """
    ...
    for group_name, sample_files in sample_groups.items():
        ...
        logger.info(f"Matrix shape for {group_name}: {matrix.shape}")
