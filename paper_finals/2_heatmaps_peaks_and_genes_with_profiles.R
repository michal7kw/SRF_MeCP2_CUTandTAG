...
compute_matrix <- function(bw_files, regions, output_file, upstream=5000, downstream=5000, 
  ...
  for (i in seq_along(bw_files)) {
    ...
    tryCatch({
      # Import bigWig - the key change: don't try to set strand on RleList
      bw_data <- rtracklayer::import.bw(bw_file, as = "RleList")
      
      # Make a non-stranded copy of regions for calculations
      # This is the key fix: create a NEW object instead of trying to modify bw_data
      ...
      if (use_scale_regions) {
        ...
        matrix <- tryCatch({
          EnrichedHeatmap::normalizeToMatrix(
            bw_data,
            noStrand_regions,  # Use the non-stranded regions
          ...
          # Try an alternative method without target_ratio
          EnrichedHeatmap::normalizeToMatrix(
            bw_data,
            noStrand_regions,  # Use the non-stranded regions
            extend = c(upstream, downstream),
        ...
        # Create a non-stranded version of the regions with single-bp width
...
plot_heatmap <- function(matrix_file, output_file, title="", zmin=0, zmax=0.3, 
  ...
  # Determine colors based on sample names or provided sample_type
  if (!is.null(sample_type) && sample_type %in% names(COLORS)) {
    # If a specific sample type is provided, use that color
    profile_colors <- rep(COLORS[[sample_type]], length(unique(melted_df$Sample)))
  } else {
    # Try to match sample names to color names
    profile_colors <- unlist(COLORS)[1:length(unique(melted_df$Sample))]
    # If we have more samples than colors, recycle
    if (length(unique(melted_df$Sample)) > length(profile_colors)) {
      profile_colors <- rep(profile_colors, length.out = length(unique(melted_df$Sample)))
    }
  }
  ...
  if (use_scale_regions) {
    ...
    x_breaks <- c(1, tss_pos, tss_pos + body_bins/2, tes_pos, ncol(matrices[[1]]))
    x_labels <- c(paste0("-", matrix_data$parameters$upstream, "bp"), 
                 "TSS", "Gene Body", "TES",
                 paste0("+", matrix_data$parameters$downstream, "bp"))
    ...
    p <- ggplot(melted_df, aes(x = Position, y = Signal, color = Sample)) +
      ...
      geom_vline(xintercept = tss_pos, linetype = "dashed", alpha = 0.6) +
      geom_vline(xintercept = tes_pos, linetype = "dashed", alpha = 0.6)
  } else {
    # For reference-point (TSS-centered)
    ...
    # Add x-axis breaks and labels
    x_breaks <- c(1, middle_bin/2, middle_bin, middle_bin*1.5, ncol(matrices[[1]]))
    x_labels <- c(
      paste0("-", matrix_data$parameters$upstream, "bp"),
      paste0("-", matrix_data$parameters$upstream/2, "bp"),
      "TSS",
    ...
    p <- ggplot(melted_df, aes(x = Position, y = Signal, color = Sample)) +
      ...
      geom_vline(xintercept = middle_bin, linetype = "dashed", alpha = 0.6)
  }
  
  # Save plot and heatmap together with improved layout
  png(output_file, width = 12, height = 10, units = "in", res = 300)
  # Set up layout
  grid.newpage()
  # Create layout with profile on top (1/3) and heatmap below (2/3)
  ...
  draw(combined_ht, newpage = FALSE)
  upViewport()
  
  dev.off()
  
  log_message(paste("Heatmap with profile saved to:", output_file))
...
generate_mecp2_tss_heatmaps <- function(temp_dir, annotation_files) {
  ...
  generate_diagnostic_plots <- function() {
    ...
    for (group_name in names(sample_groups)) {
      ...
      if (is.null(matrix_file)) {
        log_message(paste("No matrix file found for", group_name, "- skipping diagnostics"), "WARNING")
        next
      }
      ...
      if (is_rds) {
        ...
        if (!file.exists(matrix_file)) {
          log_message(paste("Matrix file not found:", matrix_file), "WARNING")
          next
        }
        ...
        matrix_data <- tryCatch({
          ...
          log_message(paste("Error reading matrix file:", e$message), "ERROR")
          return(NULL)
        })
        
        if (is.null(matrix_data) || !is.list(matrix_data) || 
            is.null(matrix_data$parameters) || 
            is.null(matrix_data$parameters$use_scale_regions) ||
            !matrix_data$parameters$use_scale_regions ||
            length(matrix_data$matrices) == 0) {
          log_message(paste("Invalid or non-scale-regions matrix for", group_name), "WARNING")
          next
        }
        ...
        color_key <- ifelse(group_name == "neuron_endo", "NEU_ENDO",
        ...
        p <- ggplot(raw_signal_df, aes(x = Position, y = Signal)) +
          geom_line(color = COLORS[[color_key]]) +
