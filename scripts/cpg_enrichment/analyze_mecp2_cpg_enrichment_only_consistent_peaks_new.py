...
class MeCP2CpGAnalyzer:
    ...
    def normalize_signal(self, peaks: BedTool, depth: int) -> BedTool:
        """Normalize peak signals by sequencing depth"""
        # Convert BedTool to dataframe for manipulation
        peaks_df = peaks.to_dataframe()
        
        # Normalize the score column (usually column 5 in BED format)
        if 'score' in peaks_df.columns:
            peaks_df['score'] = peaks_df['score'] * (1e6 / depth)  # RPM normalization
        elif 'name' in peaks_df.columns:
            peaks_df['name'] = peaks_df['name'].astype(float) * (1e6 / depth)
            
        # Convert back to BedTool
        return BedTool.from_dataframe(peaks_df)
    ...
    def calculate_enrichment_chunk(self, 
                                   chunk_id: int,
                                   total_chunks: int,
                                   exo_peaks: Dict[str, BedTool],
                                   endo_peaks: Dict[str, BedTool],
                                   cpg_islands: BedTool,
...
def get_sequencing_depths(bam_dir: str) -> Dict[str, int]:
    ...
    bam_files = list(bam_dir.glob('*.dedup.bam'))
    if not bam_files:
...
def main():
    ...
    if os.path.exists(args.bam_dir):
        logger.info(f"BAM directory contents: {list(Path(args.bam_dir).glob('*.dedup.bam'))}")
    ...
    if not all_depths:
        ...
        for peak_name in exo_peaks.keys():
            ...
            logger.error(f"  {bam_name}.dedup.bam")
    ...
    for peak_name in exo_peaks.keys():
        bam_name = peak_name.replace('_peaks', '')
        logger.info(f"Peak file: {peak_name} -> BAM file: {bam_name}.dedup.bam")
