...
class MeCP2CpGAnalyzer:
    ...
    def get_peak_signal(self, peaks_dict: Dict[str, pd.DataFrame], cpg: pd.Series) -> float:
        ...
        for sample, peaks_df in peaks_dict.items():
            # Find peaks overlapping with CpG island (including 500bp flanking regions)
            overlaps = peaks_df[
                (peaks_df['chr'] == cpg['chr']) &
                (peaks_df['start'] <= cpg['end'] + CPG_WINDOW) &  # Add 500bp downstream
                (peaks_df['end'] >= cpg['start'] - CPG_WINDOW)    # Add 500bp upstream
            ]
    ...
    def calculate_enrichment_chunk(self, 
        ...
        for _, cpg in cpg_chunk.iterrows():
            # Get overlapping peaks (including 500bp flanking regions)
            exo_overlaps = {
                sample: peaks[
                    (peaks['chr'] == cpg['chr']) &
                    (peaks['start'] <= cpg['end'] + CPG_WINDOW) &  # Add 500bp downstream
                    (peaks['end'] >= cpg['start'] - CPG_WINDOW)    # Add 500bp upstream
                ]
...
def get_sequencing_depths(bam_dir: str, peaks_type: str) -> Dict[str, int]:
    """
    ...
    for bam_file in bam_files:
        try:
            ...
            depths[f"{base_name}_{peaks_type}_peaks"] = depth
...
def main():
    ...
    parser.add_argument('--peaks-type', type=str, required=True,
                       help='Type of peaks (narrow or broad)')
    ...
    exo_files = list(Path(args.exo_peaks_dir).glob(f'*.{args.peaks_type}Peak'))
    ...
    endo_files = list(Path(args.endo_peaks_dir).glob(f'*.{args.peaks_type}Peak'))
    ...
    all_depths = get_sequencing_depths(args.bam_dir, args.peaks_type)
