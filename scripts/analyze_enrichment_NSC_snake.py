            ...
            try:
                ...
                df = pd.read_csv(filepath, sep='\t', header=None,
                                ...
                                dtype={'chr': str, 'start': int, 'end': int,
                                      'signalValue': float, 'pValue': float,
                                      'qValue': float})
                ...
                df['signalValue'] = df['signalValue'].fillna(0).clip(lower=0)
                df['pValue'] = df['pValue'].fillna(1)
                df['qValue'] = df['qValue'].fillna(1)
...
def get_peaks_near_gene(gene, peaks_dict, gene_annotations, name_to_info, window=PROMOTER_WINDOW):
    ...
    try:
        ...
        print(f"Error getting peaks for gene {gene}: {str(e)}")
        return pd.DataFrame()
