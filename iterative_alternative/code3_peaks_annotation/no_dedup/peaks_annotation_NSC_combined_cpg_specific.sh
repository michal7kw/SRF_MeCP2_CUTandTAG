#!/bin/bash
#SBATCH --job-name=pa_no_dedup_NSC_combined_cpg_specific
...
#SBATCH --error="logs/peaks_annotation/no_dedup/peaks_annotation_NSC_combined_cpg_specific.err"
#SBATCH --output="logs/peaks_annotation/no_dedup/peaks_annotation_NSC_combined_cpg_specific.out"
...
for PEAK_TYPE in narrow broad; do
    ...
    Rscript "${SCRIPT_DIR}/peaks_annotation_NSC_combined_cpg_specific.R" \
        ...
        --peak-type "${PEAK_TYPE}" \
        2>&1 | tee "logs/peaks_annotation/no_dedup/peaks_annotation_NSC_combined_cpg_specific_${PEAK_TYPE}.out"
done
