...
mkdir -p ${RESULTS_DIR}/qc/fragment_sizes
mkdir -p ${RESULTS_DIR}/qc/tss_enrichment
mkdir -p ${RESULTS_DIR}/qc/frip
...
samtools view -f 0x2 ${RESULTS_DIR}/aligned/${SAMPLE}.bam | awk -F'\t' '{print sqrt($9^2)}' | \
    sort | uniq -c | awk '{print $2,$1}' > ${RESULTS_DIR}/qc/fragment_sizes/${SAMPLE}_sizes.txt
...
if [[ -f "${RESULTS_DIR}/peaks/seacr/${SAMPLE}.stringent.bed" ]]; then
    ...
    echo -e "${SAMPLE}\t${reads_in_peaks}\t${total_reads}" > ${RESULTS_DIR}/qc/frip/${SAMPLE}_frip.txt
fi 
