...
for PEAK_TYPE in narrow broad; do
    ...
    # Output paths 
    RESULTS_DIR="${WORKING_DIR}/results_5_align2_005/peaks_common_promoters_and_cpg_only_NSC_primary_${PEAK_TYPE}"
    ...
    python -u ../scripts/peaks_common/peaks_common_promoters_and_cpg_only_NSC_primary.py \
        ...
        --peaks-dir "${PEAKS_DIR}" \
        --peak-type "${PEAK_TYPE}" \
        2>&1 | tee "logs/peaks_common_promoters_and_cpg_only_NSC_primary_${PEAK_TYPE}.out"
done
