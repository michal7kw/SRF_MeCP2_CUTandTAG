...
for sample in NSCv1 NSCv2 NSCv3; do
    cp "/beegfs/scratch/ric.broccoli/kubacki.michal/SRF_MeCP2_CUTandTAG/iterative_alternative/${PEAKS_EXPERIMENT}/peaks/narrow/${sample}_narrow_peaks.filtered.narrowPeak" "${RESULTS_DIR}/exo/${sample}_peaks.narrowPeak"
done

# Copy endogenous peaks (M samples) - removed symlinks
for sample in NSCM1 NSCM2 NSCM3; do
    cp "/beegfs/scratch/ric.broccoli/kubacki.michal/SRF_MeCP2_CUTandTAG/iterative_alternative/${PEAKS_EXPERIMENT}/peaks/narrow/${sample}_narrow_peaks.filtered.narrowPeak" "${RESULTS_DIR}/endo/${sample}_peaks.narrowPeak"
done
