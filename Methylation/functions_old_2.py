...
def analyze_methylation_patterns(genes_df: pd.DataFrame, 
    ...
    """Analyze methylation patterns with improved biological validation"""
    ...
    for cell_type, expr_df in expression_data.items():
        ...
        # Calculate methylation with improved method
        logger.info("Calculating methylation levels...")
        results[cell_type] = calculate_methylation_levels(
            merged_df,
            medip_dir,
            cell_type,
            genome_fasta
        ...
        # Add QC metrics
        results[cell_type] = add_methylation_qc_metrics(results[cell_type])
        
        # Validate calculations
        validation_results = validate_methylation_calculations(
            results[cell_type],
            cell_type,
            output_dir
        )
        ...
        logger.info(f"\nValidation results for {cell_type}:")
        logger.info(f"Median promoter/gene body ratio: {validation_results['median_ratio']:.2f}")
        logger.info(f"CpG density correlation: rho={validation_results['cpg_correlation'].statistic:.2f}, "
                   f"p={validation_results['cpg_correlation'].pvalue:.2e}")
        
        # Filter low-confidence measurements
        high_conf = results[cell_type]['methylation_confidence'] >= 0.7
        logger.info(f"Retained {high_conf.sum()} high-confidence measurements "
                   f"({high_conf.sum()/len(high_conf)*100:.1f}%)")
...
def calculate_normalized_methylation(ip_values, input_values, sequence, window_size=50):
    """
    Calculate methylation levels using improved biological normalization
    
    Args:
        ip_values: MeDIP-seq IP values
        input_values: Input control values
        sequence: DNA sequence of the region
        window_size: Size of windows for local CpG density calculation
    
    Returns:
        float: Normalized methylation level (0-100%)
    """
    ...
    # Calculate local CpG density in sliding windows
    local_densities = []
    seq_len = len(sequence)
    for i in range(0, seq_len - window_size + 1, window_size//2):
        window = sequence[i:i+window_size].upper()
        local_cpg = window.count('CG')
        local_densities.append(local_cpg / window_size)
    
    # 2. Calculate normalized signals
    ...
    # 3. Calculate enrichment with local CpG density normalization
    mean_local_density = np.mean(local_densities) if local_densities else (cpg_count / len(sequence))
    if mean_local_density == 0:
        return 0
        
    # Apply MEDIPS-like normalization
    # Normalize IP signal by input and CpG density
    enrichment = (ip_mean / input_mean) / mean_local_density
    
    # Convert to percentage (0-100)
    # Use saturation curve transformation
...
def validate_methylation_calculations(df: pd.DataFrame, cell_type: str, output_dir: str):
    """
    Validate methylation calculations against known biological patterns
    ...
    # 1. Check promoter-gene body relationship
    # Typically, active genes have low promoter methylation and higher gene body methylation
    active_genes = df[df['expression_status'] != 'downregulated']
    promoter_gene_body_ratio = active_genes['promoter_methylation'] / active_genes['gene_body_methylation']
    
    plt.figure(figsize=(8, 6))
    sns.histplot(data=promoter_gene_body_ratio, bins=50)
    plt.title(f'{cell_type}: Promoter/Gene Body Methylation Ratio\nActive Genes')
    plt.xlabel('Promoter/Gene Body Methylation Ratio')
    plt.savefig(os.path.join(validation_dir, f'{cell_type}_methylation_ratio.pdf'))
    plt.close()
    
    # 2. Check CpG density correlation
    # Higher CpG density often correlates with lower methylation in promoters
    plt.figure(figsize=(8, 6))
    sns.scatterplot(data=df, x='promoter_cpg_count', y='promoter_methylation')
    plt.title(f'{cell_type}: CpG Density vs Methylation')
    plt.xlabel('CpG Count')
    plt.ylabel('Methylation Level (%)')
    plt.savefig(os.path.join(validation_dir, f'{cell_type}_cpg_correlation.pdf'))
    plt.close()
    
    return {
        'median_ratio': promoter_gene_body_ratio.median(),
        'cpg_correlation': stats.spearmanr(
            df['promoter_cpg_count'],
            df['promoter_methylation']
        )
    }
...
def add_methylation_qc_metrics(df: pd.DataFrame) -> pd.DataFrame:
    """
    Add quality control metrics for methylation analysis
    
    Args:
        df: DataFrame with methylation data
    
    Returns:
        DataFrame with added QC metrics
    """
    df = df.copy()
    
    # 1. Calculate coverage quality
    df['promoter_coverage_quality'] = df.apply(
        lambda x: min(1.0, x['promoter_cpg_count'] / 10),  # Normalize by expected CpGs
        axis=1
    )
    
    df['gene_body_coverage_quality'] = df.apply(
        lambda x: min(1.0, x['gene_body_cpg_count'] / 20),  # Normalize by expected CpGs
        axis=1
    )
    
    # 2. Add methylation confidence scores
    df['methylation_confidence'] = df.apply(
        lambda x: calculate_methylation_confidence(
            x['promoter_methylation'],
            x['gene_body_methylation'],
            x['promoter_coverage_quality'],
            x['gene_body_coverage_quality']
        ),
        axis=1
    )
...
def calculate_methylation_confidence(prom_meth: float, gene_meth: float, 
                                  prom_cov: float, gene_cov: float) -> float:
    """Calculate confidence score for methylation measurements"""
    # Combine coverage quality and biological plausibility
    confidence = (prom_cov + gene_cov) / 2
    
    # Reduce confidence for extreme or implausible values
    if prom_meth > 90 and gene_meth < 10:
        confidence *= 0.5
    elif prom_meth < 10 and gene_meth > 90:
        confidence *= 0.5
