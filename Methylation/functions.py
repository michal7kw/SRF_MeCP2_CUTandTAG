...
def calculate_contextual_methylation(merged_df: pd.DataFrame,
    ...
    """Calculate methylation levels with genomic context"""
    # Calculate promoter methylation
    logger.info("Calculating promoter methylation...")
    promoter_methylation = calculate_methylation_levels(
    ...
    gene_body_methylation = calculate_methylation_levels(
    ...
    # Create base result DataFrame with required columns
    ...
    # Add MeCP2 binding information if available
    mecp2_columns = {
        'mecp2_bound': 'mecp2_bound',
        'binding_type': 'binding_type',
        'exo_signal': 'exo_signal',
        'endo_signal': 'endo_signal'
    }
    
    for col, source_col in mecp2_columns.items():
        if source_col in merged_df.columns:
            result_df[col] = merged_df[source_col]
        else:
            if col == 'mecp2_bound':
                result_df[col] = False
            else:
    ...
    logger.info(f"Created methylation results DataFrame with shape: {result_df.shape}")
